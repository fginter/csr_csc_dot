...
void fast_dot(idx_type m1_start, idx_type m1_slice, idx_type *m1_row_ptr, idx_type *m1_indices, idx_type *m2_col_ptr, idx_type *m2_indices, val_type *out, idx_type out_col_number) {
	...
	for (m1_ci=m1_row_ptr[m1_ri]; m1_ci<m1_row_ptr[m1_ri+1]; m1_ci++) { //for every column on that row
		...
		out[out_col_number*out_row+out_col]+=1.0;
	    }
	}
    }//m1 row index loop
}
